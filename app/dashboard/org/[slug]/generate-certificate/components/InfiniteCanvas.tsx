...
export function InfiniteCanvas({
  ...
  useEffect(() => {
    ...
    const onMove = (e: MouseEvent) => {
      if (!onTemplateResize || !resizeCorner.current) return;
      ...
      onTemplateResize(Math.max(100, nw), Math.max(100, nh));
    };
    const onUp = () => { setIsResizingTemplate(false); resizeCorner.current = null; };
  ...
  return (
    <div
      ...
      onMouseUp={() => { setIsPanning(false); setIsResizingTemplate(false); }}
      onMouseLeave={() => { setIsPanning(false); setIsResizingTemplate(false); }}
    >

      {/* ── Page Navigation (multi-page PDF) ── */}
      {totalPages > 1 && (
        <div className="absolute top-3 left-1/2 -translate-x-1/2 z-50 flex items-center gap-1 bg-white/90 dark:bg-[#1e2130]/90 backdrop-blur-sm border border-black/10 dark:border-white/10 px-2 py-1 rounded-lg shadow-lg">
          <Button
            variant="ghost" size="icon" className="h-6 w-6 text-gray-500 hover:text-gray-900 hover:bg-black/5 dark:text-white/70 dark:hover:text-white dark:hover:bg-white/10"
            onClick={() => onPageChange?.(Math.max(1, currentPage - 1))}
            disabled={currentPage <= 1}
          >
            <ChevronLeft className="w-3.5 h-3.5" />
          </Button>
          <span className="text-xs font-medium text-gray-700 dark:text-white/80 px-1 min-w-[72px] text-center">
      ...
      >
        ...
        <div
          ...
          style={{ border: '1.5px solid rgba(63,207,142,0.7)', boxSizing: 'border-box' }}
        />
        ...
        {onTemplateResize && (
          <div className="absolute inset-0 pointer-events-none z-30">
            {(['nw', 'ne', 'sw', 'se'] as const).map(corner => (
              <div
                key={corner}
                className={`absolute w-2.5 h-2.5 rounded-sm border border-white pointer-events-auto cursor-${corner === 'nw' || corner === 'se' ? 'nwse' : 'nesw'}-resize hover:scale-125 transition-transform`}
                style={{
                ...
                data-resize-handle
                onMouseDown={e => handleTemplateResizeStart(e, corner)}
              />
            ))}
          </div>
        )}
      ...
      {toolbarPos && (
        <div
          data-toolbar
          className="absolute z-50 flex items-center gap-0.5 bg-white/95 dark:bg-[#1e2130]/95 backdrop-blur-md border border-black/10 dark:border-white/10 rounded-xl shadow-2xl px-2 py-1.5"
        ...
        >
          ...
          <div
            data-grip
            className="text-gray-300 hover:text-gray-500 dark:text-white/30 dark:hover:text-white/60 cursor-grab active:cursor-grabbing px-1 mr-1"
            title="Drag to move"
          >
          ...
          <Button
            ...
            className="h-7 w-7 text-gray-500 hover:text-gray-900 hover:bg-black/5 dark:text-white/70 dark:hover:text-white dark:hover:bg-white/10 rounded-lg"
          ...
          <div className="relative">
            <button
              className="flex items-center gap-0.5 text-xs font-medium text-gray-600 hover:text-gray-900 hover:bg-black/5 dark:text-white/80 dark:hover:text-white dark:hover:bg-white/10 rounded-lg px-2 h-7 min-w-[58px] justify-center transition-colors"
            ...
            {showZoomMenu && (
              <div className="absolute bottom-full mb-2 left-1/2 -translate-x-1/2 bg-white dark:bg-[#1e2130] border border-black/10 dark:border-white/10 rounded-lg shadow-2xl py-1 min-w-[90px] z-60">
                {ZOOM_PRESETS.map(p => (
                  <button
                    key={p}
                    className={`w-full text-left px-3 py-1 text-xs hover:bg-black/5 dark:hover:bg-white/10 transition-colors ${Math.round(scale * 100) === Math.round(p * 100) ? 'text-[#3fcf8e]' : 'text-gray-600 dark:text-white/70'}`}
                    onClick={() => zoomTo(p)}
                  >
                    {Math.round(p * 100)}%
                  </button>
                ...
                <button
                  className="w-full text-left px-3 py-1 text-xs text-gray-600 dark:text-white/70 hover:bg-black/5 dark:hover:bg-white/10 hover:text-gray-900 dark:hover:text-white transition-colors"
                  onClick={fitToScreen}
                >
                  Fit to screen
                </button>
          ...
          <Button
            variant="ghost" size="icon"
            className={`h-7 w-7 rounded-lg transition-colors ${snapToGrid ? 'text-[#3fcf8e] bg-[#3fcf8e]/15 hover:bg-[#3fcf8e]/25' : 'text-gray-500 hover:text-gray-900 hover:bg-black/5 dark:text-white/70 dark:hover:text-white dark:hover:bg-white/10'}`}
      ...
      <div className="absolute bottom-3 left-3 z-40 flex items-center gap-2 text-[10px] text-gray-400 dark:text-white/30 select-none pointer-events-none">
        <span>{Math.round(pdfWidth)} × {Math.round(pdfHeight)}</span>
        <span className="w-px h-2.5 bg-black/10 dark:bg-white/15" />
        <span>{visibleFields.length} field{visibleFields.length !== 1 ? 's' : ''}</span>
        {snapToGrid && (
          <>
            <span className="w-px h-2.5 bg-black/10 dark:bg-white/15" />
            <span className="text-[#3fcf8e]/70">Snap {SNAP_SIZE}px</span>
          </>
        )}
      </div>

      {/* ── Keyboard hint (bottom-right) ── */}
      <div className="absolute bottom-3 right-3 z-40 text-[9px] text-gray-300 dark:text-white/20 select-none pointer-events-none text-right leading-relaxed">
