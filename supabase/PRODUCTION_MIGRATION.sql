...
--
-- FIXES APPLIED:
-- ✅ Category modeling consistency (certificates use FK not TEXT)
-- ✅ All security hardening from review feedback
-- ✅ Zero-trust RLS with bcrypt API keys
...
COMMENT ON COLUMN public.certificates.category_id IS
  'FK to template_categories (consistent with templates)';
...
-- Add FK constraint to certificates (CONSISTENCY FIX)
ALTER TABLE public.certificates
  DROP CONSTRAINT IF EXISTS fk_certificates_category;
...
BEGIN
  ...
  -- Store bcrypt hash (NEVER store plaintext)
  UPDATE public.companies
  SET
...
COMMENT ON FUNCTION public.generate_api_key IS
  'Generates new API key for company (returns plaintext ONCE - store hash only)';
...
COMMENT ON FUNCTION public.verify_api_key IS
  'Verifies API key via bcrypt and returns company_id if valid';
...
BEGIN
  ...
  SELECT
    ...
    co.logo,
    CASE
      WHEN c.deleted_at IS NOT NULL THEN 'not_found'
...
COMMENT ON FUNCTION public.verify_certificate IS
  'Public function to verify certificate by token (returns company info + validation result)';
...
BEGIN
  ...
  RAISE NOTICE '  ✓ RLS enabled on all 15 tables';
  ...
  RAISE NOTICE '  ✓ Category consistency: certificates → FK (not TEXT)';
  RAISE NOTICE '';
  ...
  RAISE NOTICE '  4. Configure template fields (currently empty arrays)';
  RAISE NOTICE '';
  RAISE NOTICE '━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━';
  RAISE NOTICE '';
END $$;
