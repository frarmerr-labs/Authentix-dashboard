...
- **2026-03-21** | Generate-certificate UX overhaul (6 changes) | (1) Direct navigation to design step: `handleTemplateSelect` navigates immediately, loading overlay shown during background fetch, stale-request guard via `selectRequestRef`; (2) Delete template added to TemplateSelector card hover (Trash2 icon + confirm dialog + deletingId spinner); (3) Column warning now excludes `image` + `custom_text` field types from missing-column check (DataSelector line ~91); (4) Preview opens in Dialog modal with carousel (ChevronLeft/Right + dots) instead of new tab; (5) Blueprint SVG generation animation (grid pattern + strokeDashoffset + scan line driven by progress%); (6) Native anchor click for download — no fetch/blob buffering. **Files:** `page.tsx`, `TemplateSelector.tsx`, `DataSelector.tsx`, `ExportSection.tsx`, `CertificateTable.tsx` in generate-certificate.
- **2026-03-21** | Fix 8 generate-certificate bugs | (1) Dark theme default: `className="dark"` on `<html>` in `layout.tsx`; (2) Skip template step on `?template=` URL param — initialize `currentStep`/`isTemplateLoading` from param; (3) Template resize dimension bug — partial fix pending; (4) ManualDataEntry: removed Save icon, added CheckCircle2 confirm + Enter key auto-commit + date picker auto-commit + destructive validation error styling; (5) ExpiryDateSelector custom date picker spacing + shadcn Calendar; (6) Preview modal: hide shadcn default close button via `[&>button:last-child]:hidden`; (7) Generation animation replaced with stacked-card animation + time-elapsed progress simulation (fixes 0% → 100% jump for single recipient); (8) Generate More reset now clears progress state. **Files:** `layout.tsx`, `page.tsx`, `ManualDataEntry.tsx`, `ExpiryDateSelector.tsx`, `ExportSection.tsx`, `DraggableField.tsx`.
- **2026-03-21** | Fix field selection box visually oversized + font too small | Two root causes: (1) `DraggableField.tsx` selection outline was hardcoded `#d4d4d4` (light grey) — on dark canvas looks bloated; changed to `var(--primary)` (brand green) for both `isSelected` (dashed) and `isMultiSelected` (solid); (2) `handleTemplateResize` in `page.tsx` scaled `x/y/width/height` but NOT `fontSize` — after canvas resize, boxes grew/shrank but text stayed same size; fixed by adding `fontSize: Math.round(f.fontSize * Math.sqrt(sx * sy))` (geometric mean scale factor). **Files:** `DraggableField.tsx`, `page.tsx`.
